...
public final class DigBlockResolver {
    ...
    public static Block getDigBlock(Level level) {
        ...
        if (id == null) {
            BetterEnemiesAI.LOGGER.warn("EnemyBreak_N_Build: невалідний ID блоку в конфізі: '{}', fallback на булижник", configuredId);
        ...
        if (block == Blocks.AIR && !id.equals(ResourceLocation.withDefaultNamespace("air"))) {
            ...
            BetterEnemiesAI.LOGGER.warn("EnemyBreak_N_Build: блок '{}' не знайдено в реєстрі, fallback на булижник", configuredId);
            return Blocks.COBBLESTONE;
        }
        return block;
    }
